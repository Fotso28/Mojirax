...
export function ApplyModal({ projectId, projectName, isOpen, onClose, onSuccess, onIncompleteProfile }: ApplyModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (status === 409) {
                showToast('Vous avez déjà postulé à ce projet', 'error');
            } else if (status === 400 && data?.code === 'INCOMPLETE_PROFILE') {
                onClose();
                onIncompleteProfile?.(data.missingFields || []);
            } else if (status === 403) {
                showToast(data?.message || 'Action non autorisée', 'error');
            } else if (status === 400) {
                        ...
                        <div className="px-6 py-4 space-y-4">
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Presentez-vous et expliquez pourquoi ce projet vous interesse..."
                                    rows={5}

...
import {
  ...
  Tags, Plus, Pencil, Trash2, ChevronUp, ChevronDown, X, Star,
} from 'lucide-react';
import {
...
interface Kpis {
  users: { total: number; admins: number; founders: number; candidates: number; unassigned: number; newThisWeek: number };
  projects: { total: number; published: number; draft: number; pendingAi: number; analyzingDoc: number; rejected: number };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{plans.length} plan(s) configuré(s)</p>
        </div>
        <button
          onClick={openCreate}

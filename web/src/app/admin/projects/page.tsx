...
export default function AdminProjectsPage() {
  ...
  // Charger les KPIs depuis le dashboard existant
  useEffect(() => {
    api.get('/admin/kpis')
      .then((res) => setKpis({
  ...
  const kpiCards = kpis ? [
    { label: 'Total', value: kpis.total, icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Publiés', value: kpis.published, icon: FileText, color: 'text-green-600', bg: 'bg-green-50' },
    { label: 'En attente', value: kpis.pendingAi, icon: Eye, color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: 'Rejetés aujourd\'hui', value: kpis.rejected, icon: Briefcase, color: 'text-red-600', bg: 'bg-red-50' },
  ] : [];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">Projets</h1>
        <p className="text-sm text-gray-500 mt-1">{total} projet(s) au total</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Rechercher par nom ou pitch..."
            value={search}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="text-center px-3 py-3 font-medium text-gray-500" title="Vues"><Eye className="w-4 h-4 mx-auto" /></th>
                <th className="text-center px-3 py-3 font-medium text-gray-500" title="Clics"><MousePointerClick className="w-4 h-4 mx-auto" /></th>
                <th className="text-center px-3 py-3 font-medium text-gray-500" title="Candidatures"><FileText className="w-4 h-4 mx-auto" /></th>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-500">{p.founder.name || 'Sans nom'}{p.sector ? ` · ${p.sector}` : ''}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4">

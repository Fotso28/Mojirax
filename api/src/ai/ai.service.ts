...
export class AiService {
    ...
    constructor(
        ...
        @Inject(forwardRef(() => AiConfigService)) private aiConfigService: AiConfigService,
    ) {
        ...
        // Seed les prompts par défaut en DB (fire-and-forget)
        this.seedDefaultPrompts().catch((e) => this.logger.warn(`Seed prompts failed: ${e.message}`));
    }

    private async seedDefaultPrompts() {
        await this.aiConfigService.seedPromptIfMissing('EXTRACTION', EXTRACTION_PROMPT);
        await this.aiConfigService.seedPromptIfMissing('SUMMARY', SUMMARY_PROMPT);
        await this.aiConfigService.seedPromptIfMissing('REGENERATION', 'TEMPLATE — construit dynamiquement avec blockKey et currentBlocks');
        await this.aiConfigService.seedPromptIfMissing('LEGALITY', 'TEMPLATE — construit dynamiquement avec projectData');
        await this.aiConfigService.seedPromptIfMissing('PROJECT_VALIDATION', 'TEMPLATE — construit dynamiquement avec projectData');
        await this.aiConfigService.seedPromptIfMissing('CANDIDATE_VALIDATION', 'TEMPLATE — construit dynamiquement avec profileData');
        await this.aiConfigService.seedPromptIfMissing('EMBEDDING', 'N/A — appel direct au modèle d\'embeddings');
    }
    ...
    private async extractWithOpenAI(
        ...
        let inputTokens: number | undefined;
        let outputTokens: number | undefined;
        const provider = model === 'deepseek-chat' ? 'DEEPSEEK' : 'GPT';
    ...
    async validateProject(projectData: Record<string, any>): Promise<ValidationFeedback> {
        ...
        for (const provider of this.providers) {
            try {
                ...
                const result = await this.promptProvider(provider.name, prompt, 'PROJECT_VALIDATION', 'PROJECT');
    ...
    async checkLegality(projectData: Record<string, any>): Promise<LegalityCheckResult> {
        ...
        for (const provider of this.providers) {
            try {
                ...
                const result = await this.promptProvider(provider.name, prompt, 'LEGALITY', 'PROJECT');
    ...
    async validateCandidateProfile(profileData: Record<string, any>): Promise<CandidateValidationResult> {
        ...
        for (const provider of this.providers) {
            try {
                ...
                const result = await this.promptProvider(provider.name, prompt, 'CANDIDATE_VALIDATION', 'CANDIDATE');
    ...
    async getEmbedding(text: string): Promise<number[]> {
        ...
        const provider = model === 'jina-embeddings-v3' ? 'JINA' : 'OPENAI_EMBEDDING';
        const startTime = Date.now();
        let success = true;
        let errorMsg: string | undefined;
        let inputTokens: number | undefined;
        ...
        try {
            ...
            inputTokens = (response.usage as any)?.prompt_tokens || (response.usage as any)?.total_tokens;
        ...
        for (const provider of orderedProviders) {
            try {
                ...
                if (provider.name === 'claude' && mimetype === 'application/pdf') {
                    // Claude supporte nativement les PDF — appel direct avec logging manuel
                    const pdfStart = Date.now();
                    let pdfSuccess = true;
                    let pdfError: string | undefined;
                    let pdfInputTokens: number | undefined;
                    let pdfOutputTokens: number | undefined;
                    try {
                        const response = await this.anthropic!.messages.create({
                            model: 'claude-sonnet-4-5-20250929',
                            max_tokens: 4096,
                        ...
                        pdfInputTokens = response.usage?.input_tokens;
                        pdfOutputTokens = response.usage?.output_tokens;
                        const textBlock = response.content.find((c) => c.type === 'text');
                        if (!textBlock || textBlock.type !== 'text') throw new Error('No text response');
                        ...
                        this.aiConfigService.logCall({
                            ...
                            inputTokens: pdfInputTokens,
                            outputTokens: pdfOutputTokens,
                            error: pdfError,
                            resourceType: 'PROJECT',
                        }).catch((e) => this.logger.warn(`Failed to log AI call: ${e.message}`));
                    }
                    ...
                    result = await this.promptProvider(provider.name, prompt, 'SUMMARY', 'PROJECT');
                }
    ...
    async regenerateSummaryBlock(
        ...
        for (const provider of orderedProviders) {
            try {
                ...
                const result = await this.promptProvider(provider.name, prompt, 'REGENERATION', 'PROJECT');

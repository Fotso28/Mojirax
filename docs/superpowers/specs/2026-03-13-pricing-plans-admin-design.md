...
MojiraX a besoin de 4 plans tarifaires (Gratuit, Plus, Pro, Elite) affichés sur la landing page, avec gestion complète (CRUD) depuis le panel admin.
...
Tous protégés par `FirebaseAuthGuard` + vérification rôle `ADMIN`.
...
| PATCH   | `/admin/plans/reorder`   | Réordonner les plans (body: `[{id, order}]`) |
...
**UpdatePlanDto :** PartialType de CreatePlanDto

**ReorderPlansDto :**
- `plans`: `[{id: string, order: number}]`

## 3. Frontend — Section pricing (landing)

### Adaptations de `pricing-section.tsx`

- Support 4 colonnes (`grid-cols-1 md:grid-cols-2 lg:grid-cols-4`)
- Afficher le prix en EUR : `4,99 €` (format français)
...
- **Monnaie** : EUR uniquement, champ `currency` pour anticipation future
- **Prix** : type `Decimal` Prisma, sérialisé en `number` côté API
- **Endpoint public** : `/landing/plans` existant, inchangé (filtre `isActive: true`)
- **Pas de drag & drop** : boutons flèche haut/bas pour la simplicité
- **Un seul plan populaire** : si on active `isPopular` sur un plan, les autres sont désactivés automatiquement côté API
